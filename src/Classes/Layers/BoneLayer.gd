...
func get_parent_contributions(frame: int = project.current_frame):
	...
	for parent_bone in parent_chain:
		var p_params := parent_bone.get_params(frame)
		displacement += p_params.get("local_displacement", Vector2.ZERO).rotated(rotation)
		rotation += p_params.get("local_rotation", 0.0)
	return {
		"rotation": rotation,
...
func get_net_displacement(frame: int = project.current_frame) -> Vector2:
	...
	return p_contributions["displacement"] + (
		get_animated_property(frame, "local_displacement").rotated(p_contributions["rotation"])
	)
...
func _init(_project: Project, _name := "") -> void:
	params = default_bone_params()
	for param in params:
		animated_params[param] = {}
		param_properties[param] = {}
	super(_project, _name)


func serialize() -> Dictionary:
	var data := super()  # store animatable properties (and other parent class properties)
...
func deserialize(dict: Dictionary) -> void:
	...
	gizmo_offset = str_to_var(dict.get("gizmo_offset", var_to_str(gizmo_offset)))
