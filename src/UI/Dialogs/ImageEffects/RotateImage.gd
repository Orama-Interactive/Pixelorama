...
func commit_action(cel: Image, _project: Project = Global.current_project) -> void:
	var angle: float = deg2rad(
		animate_panel.get_animated_values(commit_idx, confirmed)[Animate.ANGLE]
	)
	var init_angle: float = animate_panel.get_animated_values(commit_idx, confirmed)[Animate.INITIAL_ANGLE]
